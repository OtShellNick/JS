...
function clickHandler(event) {
    ...
    showCategory(event.currentTarget.dataset.type);
}
...
//Довольно долго думал над этой задачей, пробовал множетсво вариантов
//Конечно мне кажется можно было сделать и лучше, но пока так)
function showCategory(category) {
    let text = ''; //переменная для записи разметки
    for(let el in products) { //перебираем объекты в продуктс
        if(el == category) { //сравниваем с тем что прилетело в функцию
            switch(el) { //и разбираем возможные варианты
                case el = 'phones': 
                    for(let i = 0; i < products.phones.length; i++) {
                        text += getProductMarkup(products.phones[i]);
                        product.innerHTML = text;
                    }
                        break;
                case el = 'tablets':
                    for(let i = 0; i < products.tablets.length; i++) {
                        text += getProductMarkup(products.tablets[i]);
                        product.innerHTML = text;
                    }
                    break;
                case el = 'tv':
                    for(let i = 0; i < products.tv.length; i++) {
                        text += getProductMarkup(products.tv[i]);
                        product.innerHTML = text;
                    }
                    break;
            }
